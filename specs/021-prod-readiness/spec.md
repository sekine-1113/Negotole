...
- **FR-001**: システムは POST /api/posts に対して IP ベースのレートリミットを適用し、短時間に閾値を超えたリクエストに 429 を返さなければならない
- **FR-002**: システムは管理系 API（/api/admin/\*）に対してレートリミットを適用しなければならない
...
- **レートリミットバケット**: IP アドレスをキーに、時間窓内のリクエスト数を管理する
