...
import { POST } from "@/app/api/admin/users/[id]/freeze/route";

function makeRequest(id: string): NextRequest {
  return new NextRequest(`http://localhost:3000/api/admin/users/${id}/freeze`, {
    method: "POST",
  });
}
...
describe("POST /api/admin/users/[id]/freeze - 認証・認可", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
...
describe("POST /api/admin/users/[id]/freeze - バリデーション", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("存在しない ID → 404", async () => {
    ...
    const res = await POST(makeRequest("999"), { params: Promise.resolve({ id: "999" }) });
  ...
  it("既に凍結済み → 409", async () => {
    ...
    mockSelect.mockReturnValueOnce({
      from: () => ({
        where: () => ({
          limit: () => Promise.resolve([{ id: 2, bannedAt: new Date() }]),
        }),
      }),
    });
...
describe("POST /api/admin/users/[id]/freeze - 正常系", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("正常凍結 → 200 { success: true }", async () => {
    mockAuth.mockResolvedValueOnce(adminSession);
    mockSelect.mockReturnValueOnce({
      from: () => ({
        where: () => ({
          limit: () => Promise.resolve([{ id: 3, bannedAt: null }]),
        }),
      }),
    });
