...
```typescript
// app_user テーブルに last_login_ip カラムを追加（または login_log テーブルを新設）
...
```typescript
// next.config.ts に Sentry を統合（withSentryConfig でラップ）
// src/instrumentation.ts でサーバー・クライアント両方を初期化
```
